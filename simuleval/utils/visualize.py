...
class Visualize:
    ...
    def make_graph(self):
        ...
        data_points = [
            (delays[i], words[i]) for i in range(len(delays))
        ]  # /1000 to make it from ms to s
        ...
        for i in range(len(data_points) - 1):
            ...
            ax1.annotate(
                ...
                xytext=(x[i], y[i]),
                arrowprops=dict(
                    facecolor=color, edgecolor=color, shrink=0, width=1, headwidth=7
                ),
            )
        ...
        ax1.set_xticks(
            [
                i * (delays[1] - delays[0])
                for i in range(0, round(delays[0] / (delays[1] - delays[0])))
            ]
            + delays
        )
        ...
        additional_text = [
            ...
            (
                "Prediction length",
                data.get("prediction_length", "N/A prediction length"),
            ),
            (
                "Sample rate",
                data.get("source", "N/A source data")[1].split(":")[1].strip(),
            ),
            (
                "Duration",
                data.get("source", "N/A source data")[3].split(":")[1].strip(),
            ),
        ]
        start_y = 1.2
        line_space = 0.05
